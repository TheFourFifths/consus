...
describe('Save Item', () => {
    ...
    it('should save an item', () => {
        return post('item/save', {
            ...
            assert.strictEqual(ItemStore.getItems()[0].status, 'SAVED');
        });
    });

    it('should fail to save an item with no item address', () => {
        return post('item/save', {}).then(() => {
            throw new Error("Promise unexpectedly fulfilled");
        }).catch(e => {
            assert.strictEqual(e.message, "Item address required to save");
        });
    });
